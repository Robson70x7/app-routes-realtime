...
export async function serachDirections(source: string, destination: string) {
  ...
  if (!sourceResponse.ok) {
    throw new Error("Não foi possível buscar o local de partida");
  }

  if (!destinationResponse.ok) {
    throw new Error("Não foi possível buscar o local de destino");
  }
...
export async function NewRoutePage({
  ...
  const result =
    source || destination ? await serachDirections(source, destination) : null;
  ...
  if (result) {
    ...
    placeSourceId = result.sourcePlaceId;
    placeDestinationId = result.destinationPlaceId;
  }
